...
class RetrievalStrategyPolicyMixin:
    ...
    def _resolve_retrieval_strategy(plan: SearchPlan) -> RetrievalStrategy:
        ...
        strategy = {
            ...
            # Raw turns are the lossless source of truth.  Atomic records remain
            # useful recall shortcuts, but must not suppress direct evidence.
            "raw_turn_score_multiplier": 1.0,
            "episode_source_weight": 1.0,
            "episode_budget_ratio": 0.40,
            "min_episode_score_ratio": 0.72,
        ...
        if question_type == "personalized_advice":
            strategy.update(
                ...
                raw_turn_score_multiplier=max(strategy.get("raw_turn_score_multiplier", 1.0), 1.05),
                ...
                episode_source_weight=max(strategy.get("episode_source_weight", 1.0), 1.0),
                episode_budget_ratio=max(strategy.get("episode_budget_ratio", 0.40), 0.48),
        ...
        if question_type == "temporal":
            strategy.update(
                ...
                episode_source_weight=max(strategy.get("episode_source_weight", 1.0), 1.0),
                episode_budget_ratio=max(strategy.get("episode_budget_ratio", 0.40), 0.42),
        ...
        if question_type == "comparison":
            strategy.update(
                ...
                episode_source_weight=max(strategy.get("episode_source_weight", 1.0), 1.0),
                episode_budget_ratio=max(strategy.get("episode_budget_ratio", 0.40), 0.40),
