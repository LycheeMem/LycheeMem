    ...
    def _resolve_retrieval_strategy(plan: SearchPlan) -> RetrievalStrategy:
        ...
        if question_type == "personalized_advice":
            strategy.update(
                ...
                episode_source_weight=max(strategy.get("episode_source_weight", 0.88), 0.95),
                episode_budget_ratio=max(strategy.get("episode_budget_ratio", 1.0 / 3.0), 0.48),
        ...
        if question_type == "temporal":
            strategy.update(
                ...
                episode_source_weight=max(strategy.get("episode_source_weight", 0.88), 0.94),
                episode_budget_ratio=max(strategy.get("episode_budget_ratio", 1.0 / 3.0), 0.42),
        ...
        if question_type == "comparison":
            strategy.update(
                ...
                episode_source_weight=max(strategy.get("episode_source_weight", 0.88), 0.92),
                episode_budget_ratio=max(strategy.get("episode_budget_ratio", 1.0 / 3.0), 0.38),
