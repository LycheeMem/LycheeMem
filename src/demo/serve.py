"""
启动 LycheeMemOS Demo 服务器。

用法:
    python -m src.demo.serve
    # 或
    python src/demo/serve.py
...
def _make_embedder(settings):
    ...
    if settings.embedding_backend == "gemini" and settings.gemini_api_key:
        from src.embedder.gemini_embedder import GeminiEmbedder
...
def main():
    ...
    print("\n" + "=" * 52)
    print("  LycheeMemOS Console Demo")
