...
你可以直接通过 pip 安装 LycheeMemory：

```bash
pip install lycheemem
```

推荐安装默认 Transformer 记忆重排器：

```bash
pip install "lycheemem[rerank]"
```

如果当前 PyPI 包提示 `does not provide the extra 'rerank'`，说明安装到的是旧版本。新版 PyPI 发布前，可以先从 GitHub master 安装：
...
安装 extra 后，重排器默认开启，并会在首次使用时自动从 Hugging Face 下载当前 v0 checkpoint：
...
基础安装仍然可用：如果没有安装 PyTorch / Transformers，或者模型下载失败，LycheeMemory 会记录 warning，本进程自动关闭 rerank，继续使用 baseline 记忆检索。需要强制关闭时设置 `EXPERIMENTAL_TRANSFORMER_RERANK=false`。指标和诊断字段见 [Transformer Reranker v0](docs/transformer_reranker_v0.md)。
