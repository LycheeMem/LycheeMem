  ...
  <tbody>
    <tr>
      ...
      <td>
        <p>(紧凑行动感知存储)</p>
        <ul>
          <li>7 类 MemoryUnit</li>
          <li>在线 Pragmatic 合成</li>
          <li>行动感知检索规划</li>
          <li>RL-ready 使用统计</li>
        </ul>
      </td>
...
语义记忆采用**行动感知紧凑编码**方案，存储层使用 SQLite（FTS5 全文检索）+ LanceDB（向量索引）。

#### 记忆单元类型

每条记忆以 `MemoryUnit` 形式存储，`memory_type` 字段区分七种语义类型：
...
每个 `MemoryUnit` 除语义文本外，还携带**行动导向元数据**（`tool_tags`、`constraint_tags`、`failure_tags`、`affordance_tags`），以及**使用统计字段**（`retrieval_count`、`action_success_count` 等），为后续强化学习阶段储备信号。

多个相关 `MemoryUnit` 可由 **Pragmatic Synthesizer** 在线合并为高密度 `SynthesizedUnit`，合并后的条目在检索排序中优先于碎片单元。

#### 四模块流水线

##### 模块一：Compact Semantic Encoding（紧凑语义编码）

三阶段串行流水线，将对话轮次转换为 `MemoryUnit` 列表：

1. **原子抽取** —— LLM 从对话中提取最小自洽事实，每条记忆独立成句。
2. **指代消解** —— 将代词和上下文依赖短语展开为完整表述，使每条 unit 脱离原始对话也能被理解。
3. **行动元数据标注** —— LLM 为每条 unit 打 `memory_type`、`tool_tags`、`constraint_tags`、`failure_tags`、`affordance_tags` 等结构化标签。

`unit_id = SHA256(normalized_text)`，天然幂等，重复内容自动去重。

##### 模块二：Pragmatic Memory Synthesis（语用记忆合成）

每次固化后在线触发：

1. FTS 检测与新 unit 文本相似的已有条目（候选池）。
2. LLM 判断候选池是否值得合并（`synthesis_judge`）。
3. 若判断为是，LLM 执行合并并生成 `SynthesizedUnit`，写入 SQLite + LanceDB；原始 unit 保留不删除。

##### 模块三：Action-Aware Retrieval Planning（行动感知检索规划）

检索前由 `ActionAwareRetrievalPlanner` 分析用户查询，输出 `RetrievalPlan`：
...
1. **FTS 全文通道** —— SQLite FTS5 关键词召回 `MemoryUnit` + `SynthesizedUnit`
...
5. **时间通道** —— 按 `RetrievalPlan.temporal_filter` 过滤时间区间内的记忆
...
首先由 `ActionAwareRetrievalPlanner` 分析用户查询，生成包含 `mode`、`semantic_queries`、`pragmatic_queries`、`tool_hints` 等字段的检索计划。随后通过五通道并行召回（FTS 全文、语义向量、归一化向量、标签过滤、时间过滤）在 SQLite + LanceDB 中取出候选，经 Scorer 按六维公式排序后，合并为 `background_context` 供下游使用。技能子查询使用 HyDE 嵌入查询技能库。
...
2. **Compact 编码固化** —— 调用 `CompactSemanticEngine.ingest_conversation()`，经三阶段编码（原子抽取 → 指代消解 → 行动元数据标注）将对话内容提取为 `MemoryUnit` 并写入 SQLite + LanceDB；随后触发在线 Pragmatic Synthesis，合并相似条目生成 `SynthesizedUnit`。
