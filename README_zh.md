...
- **[05/08/2026]** 可选 Transformer 记忆重排器 v0 已加入，默认关闭，只在显式配置后启用；在 LoCoMo 以及 LongMemEval-S / MSC-MemFuse / HotpotQA zero-shot 证据选择评测上提升 hit@10。详见 [Transformer Reranker v0](docs/transformer_reranker_v0.md)。
...
LycheeMemory 提供一个默认关闭的 Transformer 记忆重排器。它不会替代基础检索，而是在更宽的候选池中重新挑选最终 top-k，适合正确证据已经被召回、但没有进入最终结果的场景。基础安装不会强制安装 PyTorch 或 Transformers。
