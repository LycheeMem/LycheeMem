...
- **[05/13/2026]** Transformer 记忆重排器 v0 现在会在安装 `lycheemem[rerank]` 后默认启用，并自动下载 [`LycheeMem/reranker`](https://huggingface.co/LycheeMem/reranker)；如果依赖或模型不可用，会安全回退到 baseline 检索。详见 [Transformer Reranker v0](docs/transformer_reranker_v0.md)。
